...
function setStatusText(id, text, status) {
  const el = document.getElementById(id);
  if (!el) return;
...
function calculateChallenge(mmr, mmrHistory) {
  ...
  const neededMatchesPerDay =
    neededMatches === null || daysLeft <= 0
      ? null
      : Math.ceil((neededMatches / daysLeft) * 10) / 10;
...
function buildArrivalResult(challenge, playDaysPerWeek, matchesPerDay) {
  ...
  return {
    ...
    note: formatArrivalDate(arrival),
    resultText: canReachByDeadline ? "チャレンジ成功" : "チャレンジ失敗"
  };
}

function calculateBaseArrivalDate(challenge) {
  return buildArrivalResult(challenge, 4, 4);
}

function calculatePaceArrivalDate(challenge) {
  ...
  return buildArrivalResult(challenge, playDaysPerWeek, matchesPerDay);
}
...
function renderChallenge(challenge) {
  ...
  if (challenge.neededMatchesPerDay === null) {
    setText("neededMatchesPerDay", "∞");
  } else {
    setText("neededMatchesPerDay", challenge.neededMatchesPerDay);
  }

  const baseResult = calculateBaseArrivalDate(challenge);
  const paceResult = calculatePaceArrivalDate(challenge);

  const statusEl = getEl("challengeStatus");
  if (statusEl) {
    statusEl.textContent = baseResult.status;
    statusEl.classList.remove("ok", "ng");

    if (baseResult.status === "到達可能" || baseResult.status === "到達済み") {
      statusEl.classList.add("ok");
    }

    if (baseResult.status === "到達不可") {
      statusEl.classList.add("ng");
    }
  }

  setText("challengeNote", baseResult.note);

  setText("paceArrivalDate", paceResult.note);
